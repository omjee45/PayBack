...
// ─────────────────────────────────────────────────────────────────────────────
// Shared seed logic — used by both prisma/seed.js (CLI) and POST /api/cron/reset
// Shreeji Textiles — 18 debtors · 55 invoices · 14 promises
// ─────────────────────────────────────────────────────────────────────────────
...
async function createBrokenPending(invId, promiseId, debtorId, debtor, amtPaise, dueDaysAgo, replyText, pastDays, confidence) {
  ...
  const promisedDate = daysAgo(pastDays);
  const captureDaysAgo = pastDays + 2;
  await addEvent(invId, 'STATE_CHANGE', { from: 'DUE', to: 'GENTLE', reason: '1_days_overdue' }, dueDaysAgo - 1);
  await addReminder(invId, 'GENTLE', debtor.preferredLanguage, M.gentle(debtor.name, amtPaise, debtor.preferredLanguage), dueDaysAgo - 1);
  await addEvent(invId, 'REMINDER_SENT', { tier: 'GENTLE' }, dueDaysAgo - 1);
  ...
  await addEvent(invId, 'PROMISE_CAPTURED', { promiseId, promisedDate: promisedDate.toISOString(), confidence }, captureDaysAgo);
}
...
// ── Main reseed function (exported for API use) ────────────────────────────
...
async function reseedDatabase() {
  ...
  // Group C — PROMISED, future date (8)
  const GC = [
  ...
  // Group D — PROMISED, PAST date (6) — ⚡ breaks on first tick
  const GD = [
